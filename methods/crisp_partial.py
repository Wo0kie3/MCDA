...
def crisp_partial(credibility, output = False):
    ...
    problem_relations = [{"var": P_plus, "rel": 'PP'}, {"var": P_minus, "rel": 'NP'}, {"var": I, "rel": 'I'}, {"var": R, "rel": 'R'}]

    FN = 0
    for i in range(n):
        for k in range(i+1,n):
            for p_relation in problem_relations:
                FN += (p_relation["var"][i, k] * RELATION_SCORES[get_relation(credibility[i][k], credibility[k][i])][p_relation["rel"]])
